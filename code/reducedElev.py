...
for files in ("MERIT_N60-N30_E060-E090.nc", "MERIT_N60-N30_E090-E120.nc",
    ...
    elev_reduced = np.nan_to_num(elev_reduced_modi, nan=-32767)

    ds = Dataset(path + f"Reduced_{files}", 'a')
    ds['Elevation'][:] = elev_reduced
    # ds['altitude'][:] = elev_reduced
    ds.close()

    cdo = Cdo()
    cdo.setctomiss('-32767', input=f'{path}Reduced_{files}', output=f'{path}reducedElev_{files}')

    del ds
    del elev_reduced_modi, elev_reduced
